#include <PololuWheelEncoders.h> //From Popolu AVR library found here https://www.pololu.com/docs/0J20
#include <NewPing.h> //From NewPing lirary found here http://playground.arduino.cc/Code/NewPing
//changed TIMER_ENABLED to false in NewPing.h file
#include <math.h>
...
#define SPEED_ADJUST_MULTIPLIER 5 //Adjustment multiplier for changing speed based on encoder readings
...
//Find closest distance that is not zero
...
int* rotate(float angle, int speed, bool direction, bool scan){
  ...
  //sets all distance readings to zero
  ...
  //set direction and disengages brake for motors
  ...
  || encoders_get_counts_m2()*(direction ? -1 : 1) > startE2*(direction ? -1 : 1) - (angle/360 * MEASURED_ENCODER_FULL_TURN)){
    ...
    //sets speed and ajusts depending on encoder values
    ...
    //reads encoder value and sets distance array at encoder value to current distance read
  ...
  //engages breaks for both motors
  digitalWrite(9, HIGH);
  digitalWrite(8, HIGH);
...
void moveTo(int speed, int distance){
  ...
  //set forward direction and disengages brake for motors
